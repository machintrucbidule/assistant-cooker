  ...
  async updateFromAttributes(withdrawalTemp, remainingTime, state, tempHistory, ambientHistory) {
    ...
    try {
      ...
      if (!this._chart.w || !this._chart.w.config || !this._chart.w.config.series) {
        // Chart not ready yet, skip this update (will be called again on next state change)
        return;
      }
      ...
      if (graphSpan === "auto") {
        ...
        if (state === "cooking" && cookingStartMs && remainingTime !== null && remainingTime > 0 && remainingTime < 720) {
          const totalDuration = (now - cookingStartMs) / 60000 + remainingTime;
          spanMinutes = Math.ceil(totalDuration * 1.1);
        } else if (state === "cooking" && cookingStartMs) {
      ...
      try {
        ...
        // Chart not ready yet, skip this update silently
        return;
      }
    } catch (e) {
